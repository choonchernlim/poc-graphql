...
def execute_fetchall(query: str, params: tuple = (), mapper: Optional[Callable[[sqlite3.Row], Any]] = None) -> List[
    Any]:
...
def execute_fetchone(query: str, params: tuple = (), mapper: Optional[Callable[[sqlite3.Row], Any]] = None) -> Optional[
    Any]:
...
def init_db():
    if os.path.exists(DATABASE_NAME):
        logging.info(f"Removing existing database file: {DATABASE_NAME}")
...
def get_accounts(user_id: strawberry.ID) -> List[Account]:
    ...
    return execute_fetchall("SELECT id, user_id, name FROM account WHERE user_id = ?", (user_id,),
                            mapper=_row_to_account)
